...
VICTIMS_KEYWORDS = [
    ...
    "сжеч", "сожж", "поджог", "поджег", "кров", "задерж", "столкнов", "катер",
    "скал", "фуры", "колес", "колёс", "сбил", "наезд",
]
...
PHASE1_SYSTEM = """Ты строгий главный редактор позитивного журнала "ПУЛЬС ДНЯ".

Проанализируй заголовок каждой новости и строго определи ее эмоциональный сентимент (sentiment):

- "positive": радостные события, вдохновляющие истории (например: вылечился, проснулся из комы, спасся, победил), научные рекорды и прорывы, свадьбы, анонсы интересных игр/фильмов, курьёзы, добрые события.
- "negative": ЛЮБЫЕ трагедии, гибель людей, смертельные ДТП, поджоги, насилие, убийства, тяжелые преступления, кровь, теракты. ДАЖЕ ЕСЛИ заголовок написан сухим языком информационного агентства!
- "neutral": только сухие финансово-статистические или официальные сводки (например: ЦБ изменил ставки, графики работы).
...
## ШАГ 0. Праздник / "Сегодня день..."?
Новость сообщает о том, что сегодня отмечается какой-либо праздник (забавный, профессиональный, необычный — например, "Всемирный день кошек", "День любителей пива", "День ВДВ")?
→ ДА: балл +9..+10 и СТОП. Это всегда должно быть в топе.
→ НЕТ: иди на Шаг 1.
...
"8 августа - Всемирный день кошек" → +10 (праздник)
...
class LLMCurator:
    ...
    def score_batch(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            raw_headline = str(item.get("headline", "")).lower()
            has_victims_keyword = any(k in raw_headline for k in VICTIMS_KEYWORDS)

            if sentiment == "negative" or is_english(ru_title) or has_victims_keyword:
