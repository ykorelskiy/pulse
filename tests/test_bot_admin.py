...
def test_build_top_selection_keyboard():
    ...
    # Test default (show_confirm_button=False)
    ...
    # Test with show_confirm_button=True
    kbd_preview = build_top_selection_keyboard(
        issue_date="2026-08-17",
        total_count=15,
        selected_indices={2, 4},
        show_confirm_button=True,
    )
    action_row_preview = kbd_preview.inline_keyboard[-1]
    assert len(action_row_preview) == 2
    assert "🗑 Убрать выбранные (2)" in action_row_preview[0].text
    assert "confirm_publish_2026-08-17" in action_row_preview[1].callback_data
